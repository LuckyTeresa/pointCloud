...
const util = require('./node_modules/node-static/lib/node-static/util');

// Current version
const version = [0, 7, 6];

class NodeStaticServer {
    constructor(root, options) {
        ...
        this.root = path.normalize(path.resolve(root || './data'));
        console.log('root folder path: ', this.root);
        ...
        this.defaultHeaders['staticServer'] = this.serverInfo;
    ...
    serveFile(pathname, status, headers, req, res) {
        ...
        console.log('served file path: ', pathname);
        fs.stat(pathname, function (e, stat) {
            if (e) {
                console.error('Failed to serve file, error: ', e);
                that.finish(404, headers, req, res, promise);
            } else {
    ...
    finish(status, headers, req, res, promise, callback) {
        ...
        headers['staticServer'] = this.serverInfo;
        ...
        if (!status || status >= 400) {
            if (callback) {
                ...
                if (promise.listeners('error').length > 0) {
                    console.log('Emit Error: ', status, headers);
    ...
    servePath(pathname, status, headers, req, res, finish) {
        ...
        //[TS] Allow proxy caching of some static resources
        if (
            pathname.indexOf('/locales/') === 0 ||
            pathname.indexOf('/environments/') === 0 ||
            pathname.indexOf('/textures/') === 0
        ) {
            headers['cache-control'] = this.options.headers['cache-control'] + ',public';
        }

        //[TS] Allow CORS use of the viewer scripts.
        headers['Access-Control-Allow-Origin'] = '*';
    ...
    respondNoGzip(pathname, status, contentType, _headers, files, stat, req, res, finish) {
        ...
        if (files.length === 1 && byteRange.valid) {
            if (byteRange.to < length) {
                ...
                // Set Content-Range response header (we advertise initial resource size on staticServer here (stat.size))
    ...
    respond(pathname, status, _headers, files, stat, req, res, finish) {
        var contentType = _headers['Content-Type'] || mime.getType(files[0]) || 'application/octet-stream';

        var usegzip = this.options.gzip;

        var idx = files[0].lastIndexOf('.');
        var ext = idx >= 0 ? files[0].slice(idx + 1) : '';

        //[TS] Serve pack files and F2D files with gzip header.
        if (
            ext === 'gz'
        ) {
            _headers['Content-Encoding'] = 'gzip';
        ...
        //[TS] Check any optional headers passed at creation time
        if (this.options.headerConfig) {
            var extHeaders = this.options.headerConfig.extension[ext];
            if (extHeaders) {
                var list = extHeaders.headers;
                for (var j in list) {
                    _headers[j] = list[j];
                    if (j === 'Content-Encoding' && list[j] === 'gzip') usegzip = false;
                }
            }
        }
    ...
    stream(pathname, files, buffer, startByte, res, callback) {
