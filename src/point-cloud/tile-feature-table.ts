export class TileFeatureTable {
    ...
    constructor(buffer: ArrayBuffer, start: number, headerLength: number, binLength: number) {
        ...
        if (headerLength !== 0) {
            ...
            header = JSON.parse(this._arrayToString(headerData));
        } else {
            header = {};
        }
        this.header = header;
    ...
    public getData(key, count, defaultComponentType = null, defaultType = null) {
    ...
    private _arrayToString(array: Uint8Array): string {
        let str = '';
